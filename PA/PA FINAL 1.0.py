import os
import time #UNTUK DELAY LOADING
import json #UNTUK MENYIMPAN DATABASE USER
import pwinput #UNTUK MENSENSOR KATA SANDI
os.system('pip install pwinput') #UNTUK INSTALL PWINPUT

class color:  #WARNA UNTUK TEKS DAN TABLE
    g = '\033[92m'
    y = '\033[93m'
    r = '\033[91m'
    w = '\033[0m'
    b = '\033[34m'

def clear_screen():  #FUNGSI UNTUK MEMBERSIHKAN TERMINAL
    os.system('cls' if os.name == 'nt' else 'clear')

def save_users_to_json(users):  #FUNGSI UNTUK MENYIMPAN DATA PENGGUNA KE DATABASE USER (FILE JSON)
    with open('users.json', 'w') as json_file: #W = write
        json.dump(users, json_file)

def load_users_from_json():  #FUNGSI UNTUK MEMUAT DATA PENGGUNA DARI DATABASE USER (FILE JSON)
    try:
        with open('users.json', 'r') as json_file:  #R = read
            users = json.load(json_file)
        return users
    except FileNotFoundError:
        return {}

def errorhandling():  #TAMPILAN ERROR HANDLING INPUT INVALID
    clear_screen()
    print(color.r+"="*60)
    print("                    ERROR! Input Invalid")
    print("="*60)
    time.sleep(2)

def errorhandling2():    #TAMPILAN ERROR HANDLING YANG MERUPAKAN ANGKA
    clear_screen()
    print(color.r+"="*60)
    print(color.r+"               ERROR! Input harus berupa angka")
    print(color.r+"="*60)
    time.sleep(2)
    input(color.g+"                 Tekan Enter untuk kembali")
    clear_screen()

admins = {"admin" : "admin"} #DATABASE ADMIN
users = load_users_from_json()  #MEMUAT DATA USER
reservasi = {"Meja 1" : "", "Meja 2" : "", "Meja 3" : "", "Meja 4" : "", "Meja 5" : "" } #NOMOR MEJA RESERVASI
menu = {"makanan" : ["PIZZA", "BURGER", "SPAGHETTI"],    #MENU MAKANAN
        "minuman" : ["ES TEH", "KOPI SUSU", "ES JERUK"]} #MENU MINUMAN
harga = {"harga_makanan" : [60000, 35000, 40000],        #HARGA MAKANAN
         "harga_minuman" : [8000, 12000, 13000]}         #HARGA MINUMAN
    
def register():     #FITUR REGISTER UNTUK USER DI BAGIAN MAINMENU
    print(color.b + "=" * 60)
    print(color.g + "                Silahkan Registrasi Akun anda")
    print(color.b + "=" * 60)
    print(color.g + "[1] Lanjut")
    print(color.g + "[0] Kembali")
    print(color.b + "=" * 60)
    pilih = input(color.g + "Masukkan pilihan: " + color.y).lower()
    time.sleep(1)
    if pilih in ['1', 'lanjut']:
        clear_screen()
        print(color.b + "=" * 60)
        print(color.g + "                Silahkan Registrasi Akun anda")
        print(color.b + "=" * 60)
        username = input(color.g + "Masukkan username: " + color.y)
        if username in users: #MENGECEK DI DATABASE USER (JSON) APAKAH USERNAME SUDAH TERSEDIA APA BELUM
            clear_screen()
            print(color.r + "=" * 60)
            print("        Username sudah digunakan. Silahkan coba lagi")
            print(color.r + "=" * 60)
            input(color.g + "\n          Tekan Enter untuk registrasi ulang")
            clear_screen()
            register()
        password = pwinput.pwinput(color.g + "Masukkan password: " + color.y)
        users[username] = password
        save_users_to_json(users)  #MENYIMPAN DATA USER KE DATABASE(JSON)
        clear_screen()
        print(color.b + "=" * 60)
        print(color.g + "           Selamat, Akun anda berhasil terdaftar")
        print(color.b + "=" * 60)
        input(color.g + "\n          Tekan Enter untuk kembali ke menu utama")
        clear_screen()
        print(color.w + "Mengembalikan ke menu utama...")
        time.sleep(2)
        clear_screen()
        mainmenu()
    elif pilih in ['0', 'kembali']:
        clear_screen()
        mainmenu()
    else:
        errorhandling()
        clear_screen()
        register()
    
def login(): #MENU LOGIN
    salah = 0
    print(color.b+"="*60)
    print(color.g+"                      Silahkan Login")
    print(color.b+"="*60)
    print(color.g + "[1] Lanjut")
    print(color.g + "[0] Kembali")
    print(color.b + "=" * 60)
    pilih = input(color.g + "Masukkan pilihan: " + color.y).lower()
    time.sleep(1)
    if pilih in ['1', 'lanjut']:
        clear_screen()
        while True:
            print(color.b+"="*60)
            print(color.g+"                      Silahkan Login")
            print(color.b+"="*60)
            username = str(input (color.g+"Masukkan username: "+color.y))
            password = str(pwinput.pwinput(color.g+"Masukkan password: "+color.y))
            if username in users and users[username] == password:    #LOGIN SEBAGAI USER
                clear_screen()
                print(color.w+"Checking...")
                time.sleep(2)
                clear_screen()
                print(color.b+"="*60)
                print(color.g+"                      Login Berhasil")
                print(color.b+"="*60)
                input(color.g+"\n          Tekan Enter untuk masuk ke menu user")
                clear_screen()
                print(color.w+"Memasuki menu user...")
                time.sleep(2)
                clear_screen()
                usermenu()
            elif username in admins and admins[username] == password: #LOGIN SEBAGAI ADMIN
                clear_screen()
                print(color.w+"Checking...")
                time.sleep(2)
                clear_screen()
                print(color.b+"="*60)
                print(color.g+"      LOGIN BERHASIL! (anda telah login sebagai ADMIN)")
                print(color.b+"="*60)
                time.sleep(2)
                input(color.g+"\n          Tekan Enter untuk masuk ke menu ADMIN")
                clear_screen()
                print(color.w+"Memasuki menu ADMIN...")
                time.sleep(2)
                clear_screen()
                adminmenu()
            elif salah == 3:                     #JIKA GAGAL LOGIN SEBANYAK 3X, MAKA AKAN KEMBALI KE MAIN MENU
                clear_screen()
                print(color.r+"="*60)
                print("                 Anda sudah 3x gagal login")
                print(color.r+"="*60)
                time.sleep(2)
                input(color.g+"Tekan Enter untuk login ulang")
                clear_screen()
                print(color.w+"Kembali ke menu utama...")
                time.sleep(2)
                clear_screen()
                mainmenu()
            else:                                   #JIKA USERNAME ATAU PASSWORD SALAH, USER AKAN DIMINTA UNTUK MENGINPUT ULANG
                clear_screen()
                print(color.w+"Checking...")
                time.sleep(2)
                clear_screen()
                print(color.r+"="*60)
                print("""         LOGIN GAGAL! Username atau password salah
                    Silahkan coba lagi""")
                print(color.r+"="*60)
                time.sleep(2)
                salah +=1
                clear_screen()
    elif pilih in ['0', 'kembali']: 
        clear_screen()
        mainmenu()
    else:
        errorhandling()
        clear_screen()
        login()

def mainmenu():     #TAMPILAN MAIN MENU
    while True:
        print(color.b+"="*60)
        print(color.g+"""                     Selamat datang di
                          Z'Resto          """)
        print(color.b+"="*60)
        print(color.g+"                         MAIN MENU")
        print(color.b+"="*60)
        print(color.g+"    [1] Register")
        print("    [2] Login")
        print("    [3] Exit")
        print(color.b+"="*60)
        pilihan = str(input(color.g+"Pilih Menu : "+color.y))
        if pilihan == "1":
            time.sleep(1)
            clear_screen()
            register()
        elif pilihan == "2":
            time.sleep(1)
            clear_screen()
            login()
        elif pilihan == "3":
            time.sleep(1)
            clear_screen()
            print(color.b+"="*60)
            print(color.g+"          Anda keluar dari program! Terimakasih")
            print(color.b+"="*60)
            exit()
        else:
            clear_screen()
            errorhandling()
            clear_screen()

def cekmenu():   #MENAMPILKAN MENU RESTORAN
    global items
    print(color.b+"="*60)
    print(color.g+"                       MENU Z'RESTO")
    print(color.b+"="*60)
    for kategori, items in menu.items():
            if kategori == "minuman":
                print(color.b+"="*60)
            print(color.y+(f"                       List {kategori}".upper()))
            print("\n             {: <5} {: <15} {: <10}".format("ID", "Nama", "Harga"))
            for i, item in enumerate(items, 1):
                harga_item = "{:,}".format(harga[f'harga_{kategori}'][i - 1])
                print(color.g+"\n             {: <5} {: <15} Rp {: <10}".format(i, item, harga_item))
    print(color.b+"="*60)

def tambah_menu():   #FUNGSI UNTUK MENAMBAHKAN MENU (FITUR ADMIN)
    while True:
        print(color.b+"="*60)
        print(color.g+"                        TAMBAH MENU")
        print(color.b+"="*60)
        print(color.g+"[1] Makanan")
        print(color.g+"[2] Minuman")
        print(color.g+"[0] Kembali")
        print(color.b+"="*60)
        pilih = input(color.g+"Masukkan pilihan: "+color.y).lower()
        if pilih not in ['1', '2', '0', 'makanan', 'minuman', 'kembali']:
            clear_screen()
            print(color.r+"="*60)
            print("                    Input tidak valid")
            print(color.r+"="*60)
            input(color.g+"         Tekan Enter untuk kembali ke Tambah Menu")
            clear_screen()
            tambah_menu()
        elif pilih in ['0', 'kembali']:
            clear_screen()
            adminmenu()
        else:
            if pilih in ['1', 'makanan']:
                tambahapa = "makanan"
            else:
                tambahapa = "minuman"
            tambah = ' '.join(input(color.g+(f"Masukkan nama {tambahapa}: "+color.y)).split()).upper()
            while tambah in menu["makanan"] or tambah in menu["minuman"]:
                print(color.r+"="*60)
                print(f"                 Error! {tambah} Sudah Tersedia")
                print(color.r+"="*60)
                time.sleep(2)
                clear_screen()
                tambah = ' '.join(input(color.g+(f"Masukkan nama {tambahapa}: "+color.y)).split()).upper()
            while True:
                try:
                    harga_menu = int(input(color.g+"Masukkan harga: "+color.y))
                    if harga_menu <= 0:
                        clear_screen()
                        print(color.r+"="*60)
                        print("   Error! Harga tidak boleh = 0 atau bernilai negatif!")
                        print(color.r+"="*60)
                        time.sleep(2)
                        clear_screen()
                        continue
                    else: 
                        menu[tambahapa].append(tambah)
                        harga[f'harga_{tambahapa}'].append(harga_menu)
                        print(color.y+(f"\nBerhasil menambahkan {tambah} seharga {harga_menu} ke dalam MENU"))
                        ulang = str(input(color.g+"\nApakah anda masih ingin menambah menu? [ya/tidak]: "+color.y)).lower()
                        while ulang not in ["ya", "tidak"]:
                            print(color.r+"\nInput tidak sesuai")
                            ulang = str(input(color.g+"\nApakah anda masih ingin menambah menu? [ya/tidak]: "+color.y)).lower()
                        if ulang == "ya":
                            clear_screen()
                            break
                        else:
                            clear_screen()
                            print(color.w+"Kembali ke menu ADMIN...")
                            time.sleep(2)
                            clear_screen()
                            adminmenu()
                except ValueError:
                    clear_screen()
                    print(color.r+"="*60)
                    print("              Error! Inputan harus berupa angka")
                    print(color.r+"="*60)
                    time.sleep(2)
                    clear_screen()
                    continue

def hapus_menu():   #FUNGSI UNTUK MENGHAPUS MENU (FITUR ADMIN)
    while True:
        print(color.b+"="*60)
        print(color.g+"                        HAPUS MENU")
        print(color.b+"="*60)
        print(color.g+"[1] Makanan")
        print(color.g+"[2] Minuman")
        print(color.g+"[0] Kembali")
        print(color.b+"="*60)
        pilih = str(input(color.g+"Masukkan pilihan: "+color.y)).lower()
        if pilih not in ['1', '2', '0', 'makanan', 'minuman', 'kembali']:
            clear_screen()
            errorhandling()
            input(color.g+"          Tekan Enter untuk kembali ke Hapus Menu")
            clear_screen()
            hapus_menu()
        elif pilih in ['0', 'kembali']:
            clear_screen()
            adminmenu()
        else:
            if pilih in ['1', 'makanan']:
                hapusapa = "makanan"
            else:
                hapusapa = "minuman"
            while True:
                try:
                    clear_screen()
                    cek_jenismenu(hapusapa)
                    hapus = int(input(color.g+(f"\nMasukkan ID {hapusapa} yang ingin dihapus: "+color.y)))
                    if hapus > len(menu[hapusapa]) or hapus <= 0:
                        clear_screen()
                        errorhandling()
                        continue
                    else:
                        hapus = menu[hapusapa].pop(hapus-1)
                        print(color.y+(f"\nBerhasil menghapus {hapus} dari MENU"))
                        ulang = str(input(color.g+"\nApakah anda masih ingin menghapus menu? [ya/tidak]: "+color.y)).lower()
                        while ulang not in ["ya", "tidak"]:
                            clear_screen()
                            errorhandling()
                            ulang = str(input(color.g+"\nApakah anda masih ingin menghapus menu? [ya/tidak]: "+color.y)).lower()
                        if ulang == "ya":
                            clear_screen()
                            cekmenu()
                            break
                        else:
                            clear_screen()
                            print(color.w+"Kembali ke menu ADMIN...")
                            time.sleep(2)
                            clear_screen()
                            adminmenu()
                except ValueError:
                    clear_screen()
                    errorhandling2()
                    continue

def ubah_harga():  #FUNGSI UNTUK MENGUBAH HARGA MENU (FITUR ADMIN)
    while True:
        print(color.b+"="*60)
        print(color.g+"                      UBAH HARGA MENU")
        print(color.b+"="*60)
        print(color.g+"[1] Makanan")
        print(color.g+"[2] Minuman")
        print(color.g+"[0] Kembali")
        print(color.b+"="*60)
        pilih = str(input(color.g+"Masukkan pilihan: "+color.y)).lower()
        if pilih not in ['1', '2', '0', 'makanan', 'minuman', 'kembali']:
            clear_screen()
            errorhandling()
            input(color.g+"          Tekan Enter untuk kembali ke Ubah Harga")
            clear_screen()
            ubah_harga()
        elif pilih in ['0', ' kembali']:
            clear_screen()
            adminmenu()
        else:
            if pilih in ['1', 'makanan']:
                ubahapa = "makanan"
            else:
                ubahapa = "minuman"
            while True:
                try:
                    clear_screen()
                    cek_jenismenu(ubahapa)
                    ubah_menu = int(input(color.g+(f"Masukkan ID {ubahapa} yang ingin diubah harganya: "+color.y)))
                    if ubah_menu > len(menu[ubahapa]) or ubah_menu <= 0:
                        clear_screen()
                        print(color.r+"="*60)
                        print("                  ERROR! ID tidak valid")
                        print(color.g+"             Silahkan masukkan ID yang valid")
                        print(color.r+"="*60)
                        time.sleep(2)
                        clear_screen()  
                        continue
                    else:
                        while True:
                            try:
                                hargabaru = int(input(color.g+("Masukkan harga baru: "+color.y)))
                                if hargabaru <= 0:
                                    print(color.r+"="*60)
                                    print("   Error! Harga tidak boleh = 0 atau bernilai negatif!")
                                    print(color.r+"="*60)
                                    time.sleep(2)
                                    clear_screen()
                                    continue
                                else:
                                    clear_screen()
                                    harga[f"harga_{ubahapa}"][ubah_menu-1] = hargabaru
                                    print(color.y+(f"Berhasil mengubah harga {menu[ubahapa][ubah_menu-1]} menjadi {hargabaru}"))
                                    time.sleep(2)
                                    ulang = str(input(color.g+"\nApakah anda masih ingin mengubah harga menu? [ya/tidak]: "+color.y)).lower()
                                    while ulang not in ["ya", "tidak"]:
                                        clear_screen()
                                        errorhandling()
                                        ulang = str(input(color.g+"\nApakah anda masih ingin mengubah harga menu? [ya/tidak]: "+color.y)).lower()
                                    if ulang == "ya":
                                        clear_screen()
                                        break
                                    else:
                                        clear_screen()
                                        print(color.w+"Kembali ke menu ADMIN...")
                                        time.sleep(2)
                                        clear_screen()
                                        adminmenu()
                            except ValueError:
                                clear_screen()
                                errorhandling2()
                                continue
                    if ulang == "ya":
                        clear_screen()
                        break
                except ValueError:
                    clear_screen()
                    errorhandling2()

def cekreservasi_admin(): #FUNGSI UNTUK MENGECEK RESERVASI (FITUR ADMIN)
    print(color.b+"="*60)
    print(color.g+"                        LIST RESERVASI")
    print(color.b+"="*60)
    for key, value in reservasi.items():
        print (color.g+(f" {key} : {value}"))
    print(color.b+"="*60)

def tambah_meja():    #FUNGSI UNTUK MENAMBAHKAN MEJA RESERVASI (FITUR ADMIN)
    print(color.b+"="*60)
    print(color.g+"                         TAMBAH MEJA")
    print(color.b+"="*60)
    print(color.g+"[1] Lanjut")
    print(color.g+"[0] Kembali")
    print(color.b+"="*60)
    pilih = str(input(color.g+"Masukkan pilihan: "+color.y)).lower()
    if pilih not in ['1', '0', 'lanjut', 'kembali']:
        errorhandling()
        input(color.g+"                Tekan Enter untuk kembali")
        clear_screen()
        tambah_meja()
    elif pilih in ['0', 'kembali']:
        clear_screen()
        adminmenu()
    else:
        while True:
            try:
                clear_screen()
                cekreservasi_admin()
                nomor_tambah = int(input(color.g+"Masukkan nomor meja baru: "+color.y))
                if nomor_tambah <= 0:
                    clear_screen()
                    print(color.r+"="*60)
                    print("  Error! Nomor Meja tidak boleh = 0 atau bernilai negatif!")
                    print(color.r+"="*60)
                    time.sleep(2)
                    clear_screen()
                    continue
                elif (f"Meja {nomor_tambah}") in list(reservasi.keys()):
                    clear_screen()
                    print(color.r+"="*60)
                    print("             Error! Nomor Meja Sudah Tersedia")
                    print(color.r+"="*60)
                    time.sleep(2)
                    clear_screen()
                    continue
                reservasi[f"Meja {nomor_tambah}"] = ""
                print(color.y+(f"\nBerhasil Menambahkan Meja {nomor_tambah} ke List Reservasi Meja"))
                while True:
                    ulang = str(input(color.g+"\nApakah anda masih ingin menambah meja? [ya/tidak]: "+color.y)).lower()
                    if ulang == "ya":
                        clear_screen()
                        break
                    elif ulang == "tidak":
                        clear_screen()
                        print(color.w+"Kembali ke menu ADMIN...")
                        time.sleep(2)
                        clear_screen()
                        adminmenu()
                    else:
                        clear_screen()
                        errorhandling()
                        clear_screen()
            except ValueError:
                clear_screen()
                errorhandling2()
                continue

def hapus_meja():   #FUNGSI UNTUK MENGHAPUS MEJA RESERVASI (FITUR ADMIN)
    print(color.b+"="*60)
    print(color.g+"                         HAPUS MEJA")
    print(color.b+"="*60)
    print(color.g+"[1] Lanjut")
    print(color.g+"[0] Kembali")
    print(color.b+"="*60)
    pilih = str(input(color.g+"Masukkan pilihan: "+color.y)).lower()
    if pilih not in ['1', '0', 'lanjut', 'kembali']:
        errorhandling()
        input(color.g+"                Tekan Enter untuk kembali")
        clear_screen()
        hapus_meja()
    elif pilih in ['0', 'kembali']:
        clear_screen()
        adminmenu()
    else:
        while True:
            try:
                clear_screen()
                cekreservasi_admin()
                nomor_hapus = int(input(color.g+"Masukkan nomor meja yang ingin dihapus: "+color.y))
                if (f"Meja {nomor_hapus}") not in list(reservasi.keys()):
                    clear_screen()
                    print(color.r+"="*60)
                    print("                Error! Nomor Meja Tidak ada")
                    print(color.r+"="*60)
                    time.sleep(2)
                    clear_screen()
                    continue
                else:
                    del(reservasi[f"Meja {nomor_hapus}"])
                    print(color.y+(f"\nBerhasil Menghapus Meja {nomor_hapus} dari List Reservasi Meja"))
                    while True:
                        ulang = str(input(color.g+"\nApakah anda masih ingin menghapus meja? [ya/tidak] : "+color.y)).lower()
                        if ulang == "ya":
                            clear_screen()
                            hapus_meja()
                        elif ulang == "tidak":
                            clear_screen()
                            print(color.w+"Kembali ke menu ADMIN...")
                            time.sleep(2)
                            clear_screen()
                            adminmenu()
                        else:
                            clear_screen()
                            errorhandling()
                            clear_screen()
            except ValueError:
                clear_screen()
                errorhandling2()
                continue

def ubah_reservasi():    #FUNGSI UNTUK MENGUBAH DATA RESERVASI (FITUR ADMIN)
    print(color.b+"="*60)
    print(color.g+"                      UBAH RESERVASI")
    print(color.b+"="*60)
    print(color.g+"[1] Lanjut")
    print(color.g+"[0] Kembali")
    print(color.b+"="*60)
    pilih = str(input(color.g+"Masukkan pilihan: "+color.y)).lower()
    if pilih not in ['1', '0', 'lanjut', 'kembali']:
        errorhandling()
        input(color.g+"                Tekan Enter untuk kembali")
        clear_screen()
        ubah_reservasi()
    elif pilih in ['0', 'kembali']:
        clear_screen()
        adminmenu()
    else:
        while True:
            try:
                clear_screen()
                cekreservasi_admin()
                nomor_ubah = int(input(color.g+"Masukkan Nomor Meja Yang Ingin Diubah Datanya: "+color.y))
                if (f"Meja {nomor_ubah}") not in list(reservasi.keys()):
                    clear_screen()
                    print(color.r+"="*60)
                    print("                Error! Nomor Meja Tidak ada")
                    print(color.r+"="*60)
                    time.sleep(2)
                    clear_screen()
                    continue
                else:
                    ubah_data = str(input(color.g+"""(Cukup Tekan Enter untuk Menghapus Nama Pereservasi)
Masukkan Perubahan Data: """+color.y))
                    reservasi[f"Meja {nomor_ubah}"] = ubah_data
                    print(color.y+(f"\nBerhasil mengubah data {f'Meja {nomor_ubah}'}\033[0m"))
                    while True:
                        ulang = str(input(color.g+"\nApakah anda masih ingin mengubah data reservasi? [ya/tidak]: "+color.y)).lower()
                        if ulang == "ya":
                            clear_screen()
                            ubah_reservasi()
                        elif ulang == "tidak":
                            clear_screen()
                            print(color.w+"Kembali ke menu ADMIN...")
                            time.sleep(2)
                            clear_screen()
                            adminmenu()
                        else:
                            clear_screen()
                            errorhandling()
            except ValueError:
                clear_screen()
                errorhandling2()
                continue

def adminmenu():  #TAMPILAN MENU ADMIN
    while True:
        print(color.b+"="*60)
        print(color.g+"                         ADMIN MENU")
        print(color.b+"="*60)
        print(color.g+"    [1] Tampilkan Menu")
        print("    [2] Tambah Menu")
        print("    [3] Hapus Menu")
        print("    [4] Ubah Harga Menu")
        print("    [5] Tampilkan List Reservasi")
        print("    [6] Tambah Meja Reservasi")
        print("    [7] Hapus Meja Reservasi")
        print("    [8] Ubah Data Reservasi")
        print("    [0] Kembali ke Main Menu")
        print(color.b+"="*60)
        pilihan = str(input(color.g+"Masukkan Pilihan : "+color.y))
        if pilihan == "1":
            clear_screen()
            print(color.w+"Loading...")
            time.sleep(2)
            clear_screen()
            cekmenu()
            input(color.g+"\n                  Tekan Enter untuk kembali")
            clear_screen()
        elif pilihan == "2":
            clear_screen()
            print(color.w+"Loading...")
            time.sleep(2)
            clear_screen()
            tambah_menu()
        elif pilihan == "3":
            clear_screen()
            print(color.w+"Loading...")
            time.sleep(2)
            clear_screen()
            cekmenu()
            hapus_menu()
        elif pilihan == "4":
            clear_screen()
            print(color.w+"Loading...")
            time.sleep(2)
            clear_screen()
            cekmenu()
            ubah_harga()
        elif pilihan == "5":
            clear_screen()
            print(color.w+"Loading...")
            time.sleep(2)
            clear_screen()
            cekreservasi_admin()
            input(color.g+"\n                  Tekan Enter untuk kembali")
            clear_screen()
        elif pilihan == "6":
            clear_screen()
            print(color.w+"Loading...")
            time.sleep(2)
            clear_screen()
            tambah_meja()
        elif pilihan == "7":
            clear_screen()
            print(color.w+"Loading...")
            time.sleep(2)
            clear_screen()
            hapus_meja()
        elif pilihan == "8":
            clear_screen()
            print(color.w+"Loading...")
            time.sleep(2)
            clear_screen()
            ubah_reservasi()
        elif pilihan == "0":
            clear_screen()
            print(color.w+"\nMengembalikan ke menu utama...")
            time.sleep(2)
            clear_screen()   
            mainmenu()
        else:
            clear_screen()
            print(color.r+"="*60)
            print("                  ERROR! Pilihan tidak ada")
            print(color.r+"="*60)
            input(color.g+"\n                 Tekan Enter untuk kembali")
            clear_screen()
            
def cek_jenismenu(jenis_pesanan): #MENAMPILKAN LIST MENU BERDASARKAN JENIS PESANAN
    print(color.b+"="*60)
    print(color.y+(f"                       List {jenis_pesanan}".upper()))
    print(color.b+"="*60)
    print(color.y+"             {: <5} {: <15} {: <10}".format("ID", "Nama", "Harga"))
    for i, item in enumerate(menu[jenis_pesanan], 1):
        harga_item = "{:,}".format(harga[f'harga_{jenis_pesanan}'][i - 1])
        print(color.g+"\n             {: <5} {: <15} Rp {: <10}".format(i, item, harga_item))
    print(color.b+"="*60)

def pesan_makanan_minuman():  #FUNGSI PEMESANAN MAKANAN/MINUMAN  (FITUR USER)
    global total_harga, pesanan
    pesanan = []
    total_harga = 0
    while True:
        cekmenu()
        print(color.g + "[1] Makanan")
        print("[2] Minuman")
        print("[0] Selesai")
        print(color.b+"="*60)
        pilih = str(input(color.g+"Apa yang ingin Anda pesan [1/2/0]? "+color.y)).lower()
        if pilih in ['0', 'selesai']:
            clear_screen()
            print(color.w+"Loading...")
            time.sleep(2)
            break
        elif pilih not in ["1", "2", "makanan", "minuman"]:
            clear_screen()
            print(color.r+"="*60)
            print("                    Pilihan tidak valid")
            print(color.r+"="*60)
            input(color.g+"            Tekan Enter untuk kembali memesan")
            clear_screen()
        else:
            if pilih in ['1', 'makanan']:
                jenis_pesanan = "makanan"
            elif pilih in ['2', 'minuman']:
                jenis_pesanan = "minuman"
            while True:
                clear_screen()
                cek_jenismenu(jenis_pesanan)
                try:
                    kode = int(input(color.g+(f"Masukkan ID {jenis_pesanan} (0 untuk kembali): "+color.y)))
                    if kode == 0:
                        clear_screen()
                        break
                    elif kode > len(menu[f"{jenis_pesanan}"]) or kode < 0: 
                        clear_screen()
                        print(color.r+"="*60)
                        print("                  ERROR! ID tidak valid")
                        print("             Silahkan masukkan ID yang valid")
                        print(color.r+"="*60)
                        time.sleep(2)
                        clear_screen()
                    else:
                        kode -= 1
                        jumlah = int(input(color.g+"Jumlah: "+color.y))
                        item = menu[f"{jenis_pesanan}"][kode]
                        harga_item = harga[f'harga_{jenis_pesanan}'][kode]
                        pesanan.append((item, harga_item, jumlah))
                        total_harga += harga_item * jumlah
                        print(color.y+(f"\n{item} (x{jumlah}): ditambahkan ke pesanan."))
                        time.sleep(1)
                except ValueError:
                    clear_screen()
                    print(color.r+"="*60)
                    print("                  ERROR! ID tidak valid")
                    print("             Silahkan masukkan ID yang valid")
                    print(color.r+"="*60)
                    time.sleep(2)
                    clear_screen()
    if len(pesanan) > 0:
        clear_screen()
        struk()
        input(color.g+"\n             Tekan Enter untuk Kembali")
        time.sleep(2)
        clear_screen()
        usermenu()
    else:
        clear_screen()
        print(color.r+"="*60)
        print(color.g+"             Anda belum memesan makanan apapun")
        print(color.r+"="*60)
        input(color.g+"\n                 Tekan Enter untuk Memesan")
        clear_screen()
        pesan_makanan_minuman()

def struk():               #STRUK PEMESANAN
    print(color.b+"="*50)
    print(color.b+"||                                              ||")
    print(f"{color.b}||                {color.g}Struk Pesanan                 {color.b}||")
    print(color.b+"||                                              ||")
    print("="*50)                               
    print(f"   {color.g}Nama pelanggan       : {color.y}{nama}")
    print(f"   {color.g}Nomor meja reservasi : {color.y}{nomor_reservasi}")
    print(color.b+"-"*50)
    print(color.g+"   {: <15}        {: <10}".format("Pesanan", "Harga"))
    for (item, harga_item, jumlah) in (pesanan):
        harga_jumlah = "{:,}".format(harga_item * jumlah)
        print(color.y+"\n   {: <3} {: <15}    Rp {: <10}".format(jumlah, item, harga_jumlah))
    print(color.b+"-"*50)
    total_struk = "{:,}".format(total_harga)
    print(f"   {color.g}Total harga          :{color.y} Rp {total_struk}")
    print(color.b+"="*50)
        
def cekreservasi_user(): #TAMPILAN MEJA RESERVASI (FITUR USER)
    print(color.b+"="*60)
    print(color.g+"                Silahkan Reservasi Meja Anda")
    print(color.b+"="*60)
    print(color.g+"                     LIST MEJA TERSEDIA")
    print(color.b+"="*60)
    for key, value in reservasi.items():
        if value == "" or "  " in value:
            print (color.g+(f"    {key}")) 
    print(color.b+"="*60)
    
def reservasi_meja():    #FUNGSI UNTUK RESERVASI MEJA (FITUR USER)
    global nama, nomor_reservasi
    while True:
        try:
            cekreservasi_user()
            nomor_reservasi = int(input(color.g+"\nMasukkan Nomor Meja yang Ingin Direservasi: "+color.y))
            if (f"Meja {nomor_reservasi}") not in list(reservasi.keys()) or reservasi[(f"Meja {nomor_reservasi}")] != "":
                clear_screen()
                print(color.r+"="*60)
                print("              ERROR! Nomor Meja Tidak Tersedia")
                print(color.r+"="*60)
                time.sleep(2)
                clear_screen()
                continue
            else:
                nama = str(input(color.g+"Silahkan masukkan nama Pereservasi: "+color.y))
                reservasi[f"Meja {nomor_reservasi}"] = nama
                clear_screen()
                print(color.b+"="*60)
                print(color.g+(f"          Berhasil Mereservasi Meja {nomor_reservasi} atas nama {nama}"))
                print(color.b+"="*60)
                break
        except ValueError:
            clear_screen()
            print(color.r+"="*60)
            print("""                           ERROR!
                Inputan harus berupa angka""")
            print(color.r+"="*60)
            time.sleep(2)
            clear_screen()
            continue

def usermenu(): #TAMPILAN MENU USER
    while True:        
        print(color.b+"="*60)
        print(color.g+"                          USER MENU")
        print(color.b+"="*60)
        print(color.g+"    [1] Reservasi")
        print("    [0] Logout")
        print(color.b+"="*60)
        pilihan = str(input(color.g+"Masukkan Pilihan : "+color.y))
        if pilihan == "1":
            clear_screen()
            print(color.w+"Memasuki menu Reservasi...")
            time.sleep(2)
            clear_screen()
            reservasi_meja()
            time.sleep(2)
            clear_screen()
            print(color.b+"="*60)
            print(color.g+"                      Silahkan Pesan")
            pesan_makanan_minuman()
            clear_screen()
            usermenu()
        if pilihan == "0":
            input(color.g+"\n          Tekan Enter untuk kembali ke menu utama")
            clear_screen()
            print(color.w+"Kembali ke menu utama...")
            time.sleep(2)
            clear_screen()
            mainmenu()
        else:
            clear_screen()
            errorhandling()
            clear_screen()

clear_screen()
mainmenu()