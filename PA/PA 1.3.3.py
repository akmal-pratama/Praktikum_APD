import os
import time #UNTUK GIMMICK TIME SLEEP
...
def save_users_to_json(users):  #FUNGSI UNTUK MENYIMPAN DATA PENGGUNA KE DATABASE(JSON)
    with open('users.json', 'w') as json_file:  #W = write
        json.dump(users, json_file)

def load_users_from_json():  #FUNGSI UNTUK MEMUAT DATA PENGGUNA DARI DATABASE(JSON)
    try:
...
def register():  #FITUR REGISTER DI BAGIAN MAINMENU
    ...
    if pilih in ['1', 'lanjut']:
        ...
        if username in users: #MENGECEK DI DATABASE(JSON) APAKAH USERNAME SUDAH TERSEDIA APA BELUM
            clear_screen()
            print(color.r + "=" * 60)
        ...
        save_users_to_json(users)  #MENYIMPAN DATA PENGGUNA KE DATABASE(JSON)
        clear_screen()
        print(color.b + "=" * 60)
...
def login(): #MENU LOGIN
    ...
    if pilih in ['1', 'lanjut']:
        ...
        while True:
            ...
            users = load_users_from_json()  #MENGECEK DI DATABASE(JSON) USERNAME TERSEDIA ATAU TIDAK
            if username in users and users[username] == password:
                ...
                clear_screen()
                adminmenu()
            elif salah == 3:                 #JIKA USER LOGIN SEBANYAK 3 KALI MAKA AKAN GAGAL LOGIN
                clear_screen()
                print(color.r+"="*60)
        ...
        while True:
            try:
                ...
                if (f"Meja {nomor_ubah}") not in list(reservasi.keys()):
                    ...
                    ubah_data = str(input(color.g+"""(Tekan Enter untuk Menghapus Nama Pereservasi)
...
def cek_jenismenu(jenis_pesanan):
    if jenis_pesanan == "makanan":  #TAMPILAN LIST MENU MAKANAN
        print(color.b+"="*60)
        ...
        print(color.b+"="*60)        
    elif jenis_pesanan == "minuman": #TAMPILAN LIST MENU MINUMAN
        print(color.b+"="*60)
...
def pesan_makanan_minuman():  #PROGRAM PESAN MAKANAN DAN MINUMAN DI FITUR USER
    ...
    while True:
        ...
        print("    [0] Selesai")
        print(color.b+"="*60)
        jenis_pesanan = str(input(color.g+"Apa yang ingin Anda pesan [0/1/2]? "+color.y)).lower()
        if jenis_pesanan in ['0', 'selesai']:
            clear_screen()
            print(color.w+"Loading...")
            time.sleep(2)
            break
        elif jenis_pesanan not in ["1", "2", "makanan", "minuman"]:
            clear_screen()
            print(color.r+"="*60)
            ...
            while True:
                try:
                    kode = int(input(color.g+(f"\nMasukkan kode {jenis_pesanan} (0 untuk kembali): "+color.y)))
                    if kode == 0:
                        clear_screen()
                        break
...
def struk():        #HASIL DARI SETELAH USER MEMILIH MAKANAN ATAU MINUMAN
    print(color.b+"="*50)
    print(color.b+"||                                              ||")
...
def cekreservasi_user(): #MENGNGECEK RESERVASI USER
    print(color.b+"="*60)
...
def reservasi_meja():    #USER DIMINTA MENGRESERVASI MEJA DI FITUR USER
    ...
    while True:
        try:
            ...
            if (f"Meja {nomor_reservasi}") not in list(reservasi.keys()) or reservasi[(f"Meja {nomor_reservasi}")] != "":
                ...
                clear_screen()
                print(color.b+"="*60)
                print(color.y+(f"          Berhasil Mereservasi Meja {nomor_reservasi} atas nama {nama}"))
                print(color.b+"="*60)
                break
        except ValueError:
            clear_screen()
