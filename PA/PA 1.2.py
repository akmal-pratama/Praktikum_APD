...
def register():
    ...
    users = load_users_from_json()  # Memuat data pengguna yang ada
    if username in users:
        print("Username sudah digunakan. Silahkan coba lagi.")
    else:
        users[username] = password
        save_users_to_json(users)  # Menyimpan data pengguna baru
...
def hapus_menu():
    ...
    while True:
        ...
        if hapusapa != "makanan" and hapusapa != "minuman":
            clear_screen()
            print(color.r+"="*60)
            print(color.r+"                    ERROR! Input Invalid")
            print(color.r+"="*60)
            time.sleep(2)
...
def struk():        
    ...
    print(color.b+"||                                              ||")
    print("="*50)                               
    print(f"{color.b}|| {color.g}Nama pelanggan       : {color.y}{nama}")
    print(f"{color.b}|| {color.g}Nomor meja reservasi : {color.y}{nomor_reservasi}")
    for (item, harga_item, jumlah) in (pesanan):
        if pesanan.index((item, harga_item, jumlah)) == 0:
            print(f"{color.b}|| {color.g}Pesanan              : {color.y}{jumlah} {item} (Rp.{harga_item * jumlah})")
        else:
            print(f"{color.b}||                        {color.y}{jumlah} {item} (Rp.{harga_item * jumlah})")
    print(color.b+"-"*50)
    print(f"{color.b}|| {color.g}Total  harga         :{color.y} Rp.{total_harga}")
    print(color.b+"="*50)
